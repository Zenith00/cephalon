...
export const boundProb = (x: number, critRate: number, failRate: number) => Math.max(critRate, Math.min(1 - failRate, x));
...
export const convolve_pmfs_prod = (pmfX_: PMF, pmfY_: PMF) => {
  ...
  const pmfY = new Map([...pmfY_.entries()].sort());
  // const jointProb = [...pmfX.entries()].map(([k, x]) => [...pmfY.entries()].map(([k, y]) => x.valueOf() * y.valueOf()));
  const pmf = new Map<number, number>() as PMF;
  [...pmfX.entries()].forEach(([xKey, xVal]) => [...pmfY.entries()].forEach(([yKey, yVal]) => {
    pmf.set(xKey * yKey, (pmf.get(xKey * yKey) || 0) + (xVal * yVal));
  }));
  return pmf;
};
...
console.log(convolve_pmfs_sum(make_pmf(6), onehit, true));
