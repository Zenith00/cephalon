...
import SpellListAside from "@/shapechange/SpellListAside";
...
export interface CouatlDatapack {
  hierarchy: {
    name: string;
    children: { name: string; named: boolean; children: { name: string }[] }[];
  };
...
const Shapechange = ({ arcane }: { arcane: string }) => {
  ...
  const debounced = useDebouncedCallback(() => {
    fetch(
      queryString.stringifyUrl(
        {
          url: `https://arcane.cephalon.xyz/${arcane}`,
        },
        { arrayFormat: "comma" }
      )
    )
  ...
  const [showNamed, setShowNamed] = useState<boolean>(false);
  ...
  const data = useMemo(
    () =>
      datapack?.hierarchy &&
      hierarchy({
        name: datapack.hierarchy.name,
        children: datapack.hierarchy.children.filter(
          (c) => showNamed || !c.named
        ),
      }),
    [datapack, showNamed]
      ...
      <AppShell
          ...
          <SpellListAside
            ...
            jumpTo={({ creature, spell }) => {
              ...
              const rootMatch = root
                ?.getAttribute("transform")
                ?.match(/translate\(([\d.]*),\s*([\d.]*)\)/);
              const [rootTranslateX, rootTranslateY] = [
                Number(rootMatch?.[2]),
                Number(rootMatch?.[1]),
              ];
              ...
              const v = {
                x: rootTranslateX - translateX,
                y: rootTranslateY - translateY,
              };
              if (!zoomRef) {
                return;
              }
              const rootPoint = zoomRef.applyInverseToPoint({
                x: rootTranslateX,
                y: rootTranslateY,
              });
              const selectedPoint = zoomRef.applyInverseToPoint({
                x: translateX,
                y: translateY,
              });

              const diff = {
                x: rootPoint.x - selectedPoint.x,
                y: rootPoint.y - selectedPoint.y,
              };
        ...
        header={
          <Header height={60} p="xs">
            <Switch
              label="Include named NPCs"
              checked={showNamed}
              onChange={(ev) => setShowNamed(ev.currentTarget.checked)}
            />
            {/* <div */}
        ...
        <ParentSize>
          {({ width, height }) => (
            <Zoom<SVGSVGElement> width={width} height={height}>
              {(zoom) =>
                (datapack && (Boolean(setZoomRef(zoom)) || true) && (
                      ...
                      {(tree) => (
                        ...
                        >
                          ...
                          {tree.descendants().map((node, i) => (
                            <Node
                              key={`node-${node.parent?.data.name ?? ""}-${
                                node.data.name
                              }`}
                              node={node as any as HierarchyPointNode<TreeNode>}
                              fullname={`${node.parent?.data.name ?? ""}|${
                                node.data.name
                              }`}
                            />
                          ))}
