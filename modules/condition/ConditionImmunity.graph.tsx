...
function ConditionImmunityGraph({
  ...
  const xMax = squareSize > margin.left + margin.right
    ? squareSize - margin.left - margin.right
    : squareSize;
  ...
  return (
    <svg width={width} height={height}>
      <HeatmapRect
        data={datapack.data}
        xScale={(d) => lpad + xScale(d) || 0}
        gap={2}
        yScale={(d) => yScale(d) ?? 0}
        binWidth={binWidth}
        binHeight={binHeight}
        colorScale={(x) => interpolateGnBu(
          (x.valueOf() / 100) * (1.0 - colorBuffer) + colorBuffer,
        )}
      >
