...
export const diceToPMFs = (dice: string): PMF[] => {
  ...
  if (face) {
    ...
    return [...Array(Number(count) || 1).keys()].map((_) =>
      make_pmf(Number(face))
    );
  }
...
export const simpleProcess = memoize(
  (damage: string, crit: critType = "none"): PMF => {
    ...
    let pmf = (dice.pos || [])
      ...
      .map((x: PMF) => x)
      .reduce(
        (acc, c) => convolve_pmfs_sum_2(acc, c, true),
        new Map([[0, ONE]])
      );
