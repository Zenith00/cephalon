...
import { useEdges, useNodes, Handle, Position } from "react-flow-renderer";
...
import { getDamagerNode, getPlayerNode } from "@damageBlocks/utils";
...
export const DamagerNode = ({
  data,
  id,
}: DamagerNodeData & EnrichedNodeProps) => {
  ...
  React.useEffect(() => {
    if (!data && sourceOnHitPMFs.length === 0) {
      return;
    }
    pool
      .proxy()
      .then((worker) => {
        ...
        // }
        const a =
          data.damager.sourcePlayer &&
          getPlayerNode(Nodes, data.damager.sourcePlayer);

        const playerData =
          data.damager.sourcePlayer &&
          (
            getPlayerNode(Nodes, data.damager.sourcePlayer)
              ?.data as PlayerMetadatum
          ).player;
        //
        // if (!playerNode) {
        //   return;
        ...
        const p = worker.computeDamageInfo(
          damager,
          "normal",
          playerData,
          sourceOnHitPMFs
      ...
      .then((res) => {
        ...
        console.log("Got!");
        console.log({ res });
        const { damagePMFByAC, toHitCumulatives } =
          res as unknown as ReturnType<typeof computeDamageInfo>;
  ...
  const updatePlayerEdges = (ontoEdges: Edge[]) => {
    const playerEdge = ontoEdges.find((edge) =>
      edge.source.startsWith("player|")
    );
    const playerNode =
      playerEdge &&
      getPlayerNode(Nodes, playerEdge.source as `player|${number}`);
    if (!playerNode) {
      return;
    }
    const player = getPlayerNode(Nodes, playerEdge.source as PlayerKey);
    if (!player) {
      return;
    }
    updateNode(id, {
      ...data,
      damager: { ...data.damager, sourcePlayer: player.id },
    });
  ...
  useEffect(() => {
    ...
    const damagerOnHitEdges = ontoEdges.filter((edge) =>
      edge.source.startsWith("damager|")
    );
    const sourceOnHitDamagerNodes = damagerOnHitEdges.map((damagerOnHitEdge) =>
      getDamagerNode(Nodes, damagerOnHitEdge.source as `damager|${number}`)
    );
        ...
        >
          ...
          {damageData?.get(1) && (
            <TextInput
              ...
              value={weighted_mean_pmf(damageData.get(1)!).toString(3)}
              size="xs"
              variant="filled"
            />
          )}
          {sourceOnHitPMFs.length === 0 && (
            <LabeledHandler
              type="target"
