...
import {
  ChevronDown, ChevronUp, MoonStars, Sun,
...
const PointBuy = () => {
  const [points, setPoints] = useState<AbilityScoreData>(
    {
  ...
  const [showCustomScores, toggleShowCustomScores] = useToggle(false, [false, true]);
  const [themeColor, toggleThemeColor] = useToggle<'dark'|'light'>('dark', ['dark', 'light']);
          ...
          <Navbar width={{ sm: 200, md: 300 }} p="xs" hiddenBreakpoint="sm" hidden={hideNavbar}>
            <Navbar.Section>
              <WheelNumberInput
                ...
                min={1}
                value={rules.points}
                onChange={(newValue) => setRules(produce((draft) => { draft.points = newValue || 1; }))}
                onWheel={(ev: React.WheelEvent) => {
                  const newValue = rules.points + (ev.deltaY < 0 ? 1 : -1);
                  setRules(produce((draft) => { draft.points = newValue; }));
                }}
              />
              <WheelNumberInput
                label="Maximum Score"
                ...
                max={20}
                min={1}
                onChange={(newValue) => setRules(produce((draft) => { draft.maxScore = newValue || 20; }))}
                onWheel={(ev: React.WheelEvent) => {
                  const newValue = rules.maxScore + (ev.deltaY < 0 ? 1 : -1);
                  if (newValue <= 20 && newValue >= 1) setRules(produce((draft) => { draft.maxScore = newValue; }));
                }}
              />
              <WheelNumberInput
                label="Minimum Score"
                ...
                max={20}
                min={1}
                onChange={(newValue) => setRules(produce((draft) => { draft.minScore = newValue || 1; }))}
                onWheel={(ev: React.WheelEvent) => {
                  const newValue = rules.minScore + (ev.deltaY < 0 ? 1 : -1);
                  if (newValue <= 20 && newValue >= 1) setRules(produce((draft) => { draft.minScore = newValue; }));
                }}
              />
            </Navbar.Section>
            <div>
                ...
                <Button onClick={() => toggleShowCustomScores()} variant="subtle" mb="md" compact>
                  <Group align="stretch">
                    <div style={{ alignItems: 'center', display: 'flex', height: '100%' }}>
                      {showCustomScores ? <ChevronUp /> : <ChevronDown />}
                      Point Costs
                    </div>
          ...
          <Header height={60} p="xs">
            ...
            >
              <Burger
                ...
                size="sm"
                style={width > theme.breakpoints.sm ? { display: 'none' } : {}}
              />
